...
public class EventActionHolder : IEquatable<EventActionHolder> {
    ...
    public bool Equals(EventActionHolder other) => ReferenceEquals(Callback, other?.Callback) && ReferenceEquals(Owner, other?.Owner);
    public override int GetHashCode() => HashCode.Combine(Callback, Owner);
}